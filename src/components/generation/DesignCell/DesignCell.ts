...
export default class Cell implements TCell {
  ...
  draw() {
    this.canvasCtx.clearRect(this.x, this.y, this.size, this.size);
  ...
  drawWalls(walls: Walls) {
    ...
    canvasCtx.lineWidth = 0.5 * this.borderWeight;

    if (this.walls[NORTH]) {
      // this.line(this.x, this.y, this.x + this.size, this.y, this.borderColor);
      // this.line(this.x, this.y, this.x + this.size, this.y, 'red');

      canvasCtx.shadowOffsetX = 0;
      canvasCtx.shadowOffsetY = 10;
      canvasCtx.shadowColor = '#aaa';
